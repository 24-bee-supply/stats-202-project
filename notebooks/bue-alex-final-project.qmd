...
Longer lenghts of query length has a very high proportion of relevant entries. I calculate standard errors to see whether these trends are statistically significant or may result in over-fitting. 
...
preprocess_data <- function(data) {
 ...
 # Remove unique identifiers
 data_processed <- data %>%
   select(-id, -query_id, -query_length)
...
apply_transformations <- function(data_processed) {
  ...
  data_transformed$log_sig6_pos
  itive <- ifelse(data_transformed$sig6 > 0, 
...
model_comparison <- data.frame(
  ...
  CV_Accuracy = c(
    round(1 - min(readRDS("../models/lasso_model.rds")$cv_model$cvm), 4),
    round(max(readRDS("../models/knn_model.rds")$results$Accuracy), 4),
    round(max(readRDS("../models/rf_model.rds")$results$Accuracy), 4)
  ),
  Optimal_Parameter = c(
    paste("λ =", round(readRDS("../models/lasso_model.rds")$cv_model$lambda.min, 4)),
    paste("k =", readRDS("../models/knn_model.rds")$bestTune$k),
    paste("mtry =", readRDS("../models/rf_model.rds")$bestTune$mtry)
  )
)
...
Code for creating the test dataframe is available in the Git Hub repository but hidden here. 

There was some difficulty applying the final Random Forest model to the test data. Random Forest, in its Caret package form, requires identical factor levels. But I cannot ascertain what factor levels appear in the test data without peaking at it. Therefore, perhaps naively, I simply imposed the same factors from the training data preprocessing to the test data. 
...
# For KNN, need to convert to numeric matrix (one-hot encoding)
X_test <- model.matrix(~ . - 1, data = test_transformed)
...
# Convert to binary
predictions_binary <- as.integer(as.character(predictions))

# Check prediction distribution
cat("KNN Prediction Distribution:\n")
cat("============================\n")
cat("Mean predictions:", round(mean(predictions_binary), 3), "\n")
...
# Save predictions
write.csv(submission, "../data/bue-alex-knn-predictions.csv", row.names = FALSE)

cat("Predictions saved to: bue-alex-knn-predictions.csv\n\n")
